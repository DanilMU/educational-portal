...
- **Основные учебные модули:** `subjects`, `topics`, `lessons`, `quizzes` еще не созданы.
...
- [x] **Задача:** В `prisma/schema.prisma` определить все модели данных: `User`, `Subject`, `Topic`, `Lesson`, `Quiz`, `Question`, `Answer`, `UserProgress`, `Certificate`.
...
- **Реализация:**
    ...
    - [ ] `auth.controller.ts`: Добавить endpoint для `/refresh`.
...
1.  **Завершить модуль `users`:** Создать необходимые DTO.
2.  **Реализовать Refresh Token:** Добавить логику обновления токенов в `auth.service.ts` и соответствующий эндпоинт.
3.  **Создать учебные модули:** Последовательно реализовать `subjects`, `topics`, `lessons`, `quizzes`.
4.  **Написать тесты:** Параллельно с разработкой покрывать новую логику юнит- и e2e-тестами.
